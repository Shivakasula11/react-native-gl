...
const WallPosts = ({ refresh }) => {
  ...
  const fetchPosts = async (offset, limit) => {
    ...
    if (error) {
      ...
      const formattedPosts = data.map(post => ({
        ...post,
        username: post.profiles?.first_name && post.profiles?.last_name
          ? `${post.profiles.first_name} ${post.profiles.last_name}`
          : 'Unknown',
        likeCount: post.wall_likes[0]?.count || 0,
        commentCount: post.wall_comments[0]?.count || 0,
        isEdited: post.edited_at && new Date(post.edited_at) > new Date(post.created_at),
  ...
  useEffect(() => {
    ...
    const likeSubscription = supabase
      .channel('wall_likes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'wall_likes' }, fetchPosts)
      .subscribe();

    const commentSubscription = supabase
      .channel('wall_comments')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'wall_comments' }, fetchPosts)
      .subscribe();
  ...
  return (
    <FlatList
      ...
      ListFooterComponent={loading ? <Text>Loading...</Text> : <View style={{ height: 50 }} />}
  ...
  const editPost = async () => {
    ...
    if (error) {
      ...
      setPosts(prevPosts =>
        prevPosts.map(p =>
          p.id === post.id
            ? { ...p, content: newContent, isEdited: true, edited_at: new Date().toISOString() }
            : p
        )
      );
      setIsEditing(false);
  ...
  return (
    <View style={styles.postContainer}>
          ...
          <View style={styles.postActions}>
            {isEditing ? (
              ...
              <>
                ...
                <TouchableOpacity onPress={handleDeletePost} style={styles.iconButton}>
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity onPress={() => setCommentModalVisible(true)} style={styles.actionButton}>
...
const LikeButton = ({ postId, likeCount }) => {
  ...
  return (
    <TouchableOpacity onPress={toggleLike} style={styles.actionButton}>
      <Ionicons name={liked ? 'thumbs-up' : 'thumbs-up-outline'} size={22} 
...
const styles = StyleSheet.create({
  ...
  media: {
    width: '100%',  // Ensures image fills the container's width
    height: undefined,  // Allow height to adjust based on the aspect ratio
